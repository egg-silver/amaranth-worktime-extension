 ...
 * members: [{ name, empCd }]
 */
export async function fetchTeamAttendance(credentials, { coCd, date, members }) {
  const list = members || [];
  ...
  const results = await Promise.all(
    list.map(async (m) => {
      try {
        const res = await fetchTodayCommute(credentials, { empCd: m.empCd, coCd, workDt: date });
        const d = res || {};
        const come = hhmm(d.comeTm);
        const leave = hhmm(d.leaveTm);
        // 근무시간은 퇴근까지 찍혔을 때만. 점심 12–13시는 겹치는 만큼 뺀다.
        let workMin = 0;
        const c = toMin(come), l = toMin(leave);
        ...
        return { name: m.name, empCd: m.empCd, comeTm: '', leaveTm: '', workMin: 0, holiday: false, ok: false };
      }
    })
  );
  return results;
