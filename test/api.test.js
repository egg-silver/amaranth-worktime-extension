...
test("세션 정보라도 진짜 오류 코드는 그대로 던진다", async () => {
  stubFetch(() => ({
    json: { resultCode: 601, resultMsg: "허용된 쿠키 인증 URL이 아닙니다." },
  }));
  await assert.rejects(
    () => fetchCompanyInfo(credentials),
    /허용된 쿠키 인증 URL/,
  );
});
...
test("다른 API 는 여전히 resultCode 0 만 성공으로 본다", async () => {
  stubFetch(() => ({
    json: { resultCode: 200, resultMsg: "이미 로그인된 사용자입니다." },
  }));
  await assert.rejects(
...
test("읽음 처리는 event02A03 에 alertIds 를 담아 보낸다", async () => {
  const calls = stubFetch(() => ({
    json: { resultCode: 0, resultData: { alertIds: [] } },
  }));
  await markAlertsRead(credentials, { compSeq: "1000", bizSeq: "1000" }, [
    "id-1",
    null,
    "id-2",
  ]);
  ...
  assert.deepEqual(JSON.parse(calls[0].init.body), {
    ...
    body: {
      companyInfo: { compSeq: "1000", bizSeq: "1000" },
      alertIds: ["id-1", "id-2"],
    },
  });
});
...
test("모두 읽음은 event02A04 다음 event02A12 를 보낸다", async () => {
  const calls = stubFetch(() => ({
    json: { resultCode: 0, resultData: { total: 0, failed: 0 } },
  }));
...
test("모두 읽음은 첫 호출이 실패하면 두 번째를 보내지 않는다", async () => {
  const calls = stubFetch(() => ({
    json: { resultCode: 1, resultMsg: "실패" },
  }));
