# 배포할 때

## 받는 사람이 할 일

1. 폴더를 통째로 받는다
2. `chrome://extensions` → **개발자 모드** 켜기
3. **압축해제된 확장 프로그램을 로드합니다** → 그 폴더 선택
4. 그룹웨어(`https://gw.goorm.io`)에 **한 번** 접속 (사번을 읽어온다)

크롬 웹스토어에 올리지 않는 한 이 방식만 가능하다.

## 실행 파일만 압축하기

```bash
npm run package
```

`dist/amaranth-worktime-extension.zip`을 생성한다. 압축을 풀면 `amaranth-worktime-extension` 폴더가 생기고 루트에 `manifest.json`이 나온다.
확장 실행에 필요한 루트 파일과 `lib/`, `fonts/`, `icons/`만 포함한다.
문서, 테스트, 개발 도구, Git 정보, `package.json`은 제외한다.
업데이터와 패키징 명령은 같은 파일 허용 목록을 사용한다. ZIP 이름에 버전을 넣지 않아 압축을 풀 때 폴더 이름이 버전별로 바뀌지 않는다.

## 개인 정보

**코드에는 사번도 토큰도 없다.** 설정 화면의 만든 사람 목록(`popup.js` 의 `CREDITS`)에만 이름이 들어간다.

각자 자기 브라우저의 그룹웨어 쿠키로만 동작하므로, 남의 근태를 볼 수 없고
내 정보가 남에게 가지도 않는다. 모든 데이터는 각자 브라우저 안에만 저장된다
(`chrome.storage.local`). 근태 조회는 `gw.goorm.io`로 통신하고, 버전 확인과 업데이트 다운로드에는 GitHub API 및 raw 파일 서버를 사용한다.

사번은 설치 후 그룹웨어에 접속할 때 **각자의 브라우저에서** 읽어 저장된다.

## 알아둘 것

이건 **비공식 확장**이다. 그룹웨어가 공개한 API가 아니라, 웹 화면이 쓰는 요청을
그대로 흉내 내서 조회한다. 조회만 하고 쓰기는 하지 않지만, 사내에서 이런 도구를
돌려도 되는지는 회사 방침에 따라 다를 수 있다. 널리 뿌리기 전에 확인해 두는 편이 낫다.

그룹웨어가 서명 방식이나 응답 형식을 바꾸면 동작이 멈춘다. 그때는 팝업의
[설정] → **연결 진단**이 어느 단계에서 막혔는지 알려준다.

## 업데이트 배포

`master` 에 머지되면 `.github/workflows/release.yml` 이 돈다. **머지 한 번에 릴리스 하나**가 나간다.
기여자가 버전을 신경 쓸 필요는 없다.

- 버전을 안 올린 채 머지하면 워크플로가 **패치 버전을 올려 커밋해 두고** 그 버전으로 낸다
- PR 에서 `manifest.json` 과 `package.json` 의 `version` 을 직접 올렸으면 그 값을 그대로 쓴다.
  minor/major 를 내고 싶을 때 이렇게 한다. 두 값이 다르면 워크플로가 멈춘다
- 릴리스 노트를 쓰려면 `release-notes/v<버전>.md` 를 같이 커밋한다. 없으면 커밋 목록으로 자동 생성된다

테스트가 깨지면 버전도 안 올라가고 릴리스도 안 나간다.
태그는 워크플로가 `--target` 으로 직접 만든다. 손으로 태그를 밀 필요 없다.
워크플로가 되밀어 넣는 버전 커밋에는 `[skip ci]` 가 붙어서 자기 자신을 다시 부르지 않는다.

폴더 업데이트는 최신 공개 릴리스의 태그가 가리키는 소스에서 루트 실행 파일과 `lib/`, `fonts/`, `icons/`를 받는다.
새 루트 실행 파일을 추가하면 `lib/folder-update.js`의 배포 파일 허용 목록도 갱신해야 한다.
다운로드·교체·폴더 저장 UI는 `update.html`, `update.js`에 있다.
